...
def count_bibliography_entries() -> int:
    """Count the number of entries in the curated bibliography file."""
    if not os.path.exists(BIBLIOGRAPHY_FILE):
        print(f"Warning: {BIBLIOGRAPHY_FILE} not found. Defaulting publication count to 0.")
        return 0
...
def get_scholar_citations() -> None:
    ...
    # Author-level summary stats (h-index, i10-index, total citations, etc.)
    citation_data["metadata"]["citedby"] = author_data.get("citedby", 0)
    citation_data["metadata"]["hindex"] = author_data.get("hindex", 0)
    citation_data["metadata"]["i10index"] = author_data.get("i10index", 0)
    # Publication count reflects the curated bibliography, not the raw Scholar
    # publication list (which includes preprints/duplicates not shown on the site).
    citation_data["metadata"]["num_publications"] = count_bibliography_entries()
    print(
        f"Author stats: {citation_data['metadata']['citedby']} citations, "
        f"h-index {citation_data['metadata']['hindex']}, "
        f"i10-index {citation_data['metadata']['i10index']}, "
        f"{citation_data['metadata']['num_publications']} publications (from {BIBLIOGRAPHY_FILE})"
    )
